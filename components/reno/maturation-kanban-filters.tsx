...
import { PHASES_KANBAN_MATURATION, MATURATION_PHASE_LABELS } from "@/lib/reno-kanban-config";
import type { ProjectRow } from "@/hooks/useSupabaseProjects";
import { trackEventWithDevice } from "@/lib/mixpanel";

export interface MaturationFilters {
...
export const DEFAULT_MATURATION_FILTERS: MaturationFilters = {
  architectNames: [],
  scouterNames: [],
  ecuStatus: "all",
  investmentType: "all",
...
export function getMaturationFilterBadgeCount(f: MaturationFilters): number {
  return (
    (f.architectNames.length > 0 ? 1 : 0) +
    (f.scouterNames.length > 0 ? 1 : 0) +
    (f.ecuStatus !== "all" ? 1 : 0) +
...
interface MaturationKanbanFiltersProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  allProjects: ProjectRow[];
  filters: MaturationFilters;
  onFiltersChange: (filters: MaturationFilters) => void;
}

export function MaturationKanbanFilters({
  open,
  onOpenChange,
  allProjects,
  filters,
  onFiltersChange,
}: MaturationKanbanFiltersProps) {
  const [local, setLocal] = useState<MaturationFilters>(filters);
  ...
  const availableScouters = useMemo(() => {
    const names = new Set<string>();
    for (const p of allProjects) {
      const sc = ((p as any).scouter ?? "").toString().trim();
      if (sc) names.add(sc);
    }
    return Array.from(names).sort();
  }, [allProjects]);
  ...
  const handleApply = () => {
    ...
    trackEventWithDevice("Maturation Filters Applied", {
      architects: local.architectNames.length,
      scouters: local.scouterNames.length,
      ecu: local.ecuStatus,
  ...
  const handleClear = () => {
    const cleared = { ...DEFAULT_MATURATION_FILTERS };
    setLocal(cleared);
    onFiltersChange(cleared);
    trackEventWithDevice("Maturation Filters Cleared");
    onOpenChange(false);
  };
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
        ...
        <div className="space-y-6 py-2">
          ...
          {/* Scouter */}
          <div className="space-y-3">
            <Label className="text-sm font-semibold">Scouter</Label>
            <div className="grid grid-cols-1 gap-2 max-h-40 overflow-y-auto">
              {availableScouters.map((name) => (
                <label key={name} className="flex items-center gap-2 cursor-pointer">
                  <Checkbox
                    checked={local.scouterNames.includes(name)}
                    onCheckedChange={(checked) => {
                      setLocal((prev) => ({
                        ...prev,
                        scouterNames: checked
                          ? [...prev.scouterNames, name]
                          : prev.scouterNames.filter((n) => n !== name),
                      }));
                    }}
                  />
                  <span className="text-sm">{name}</span>
              ...
              {availableScouters.length === 0 && (
                <span className="text-sm text-muted-foreground italic">Sin scouters disponibles</span>
              )}
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 gap-2">
              ...
              {PHASES_KANBAN_MATURATION.map((phase) => (
                <label key={phase} className="flex items-center gap-2 cursor-pointer">
                  ...
                  />
                  <span className="text-sm">{MATURATION_PHASE_LABELS[phase]}</span>
                </label>
              ))}
            </div>
          </div>
