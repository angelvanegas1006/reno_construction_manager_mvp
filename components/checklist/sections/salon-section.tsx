                            ...
                            <div className="space-y-4 pt-2">
                              ...
                              <div className="space-y-2">
                                <ChecklistUploadZoneComponent
                                  title="Fotos"
                                  description="Añade fotos del problema o elemento que necesita reparación/reemplazo"
                                  uploadZone={{ id: `${carpentryItem.id}-photos`, photos: carpentryItem.photos || [], videos: carpentryItem.videos || [] }}
                                  onUpdate={(updates) => {
                                    handleCarpentryMediaChange(carpentryItem.id, null, updates.photos, updates.videos || []);
                                  }}
                                  isRequired={true}
                                  maxFiles={10}
                                  maxSizeMB={5}
                            ...
                            <div className="space-y-4 pt-2">
                              ...
                              <div className="space-y-2">
                                <ChecklistUploadZoneComponent
                                  title="Fotos"
                                  description="Añade fotos del problema o elemento que necesita reparación/reemplazo"
                                  uploadZone={{ id: `${climatizationItem.id}-photos`, photos: climatizationItem.photos || [], videos: climatizationItem.videos || [] }}
                                  onUpdate={(updates) => {
                                    handleClimatizationMediaChange(climatizationItem.id, null, updates.photos ?? [], updates.videos ?? []);
                                  }}
                                  isRequired={true}
                                  maxFiles={10}
                                  maxSizeMB={5}
        ...
        <Card className="p-4 sm:p-6 space-y-4">
          <div className="space-y-4">
            ...
            {(mobiliario.question?.status === "buen_estado" || mobiliario.question?.status === "necesita_reparacion" || mobiliario.question?.status === "necesita_reemplazo") && (
              <div className="space-y-2">
                <Label className="text-xs sm:text-sm font-medium text-foreground leading-tight break-words">
                  {t.checklist.sections.salon.mobiliario.queMobiliarioExiste} <span className="text-danger">* <span className="ml-1">{t.formLabels.required}</span></span>
                </Label>
                <Textarea
